...
# Создание обюъекта app, который и будет наши приложением
app=Flask(__name__)
...
def login():
    if request.method == 'POST':
        if request.form.get("login"):
            ...
            if not records:
                return 'Ошибка, данные ввыдены неверно'
...
# Создаем соединение с нашей базой, где  дополнительными апараметрами являются: имя базы данных, имя пользователя, пароль, адрес хоста, номер порта
